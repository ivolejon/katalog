...
- **Stabila portar:** varje `aspire run` startar med samma portar - dashboard/AppHost `https://localhost:15000` (via den commit:ade `Katalog.AppHost/Properties/launchSettings.json` applicationUrl; notera negationen i .gitignore) och api `http://localhost:5192` + web `http://localhost:5173` (fasta endpoint-hostPortar i `Katalog.AppHost/Program.cs` och `Resources/Api/KatalogApi.cs`). Portarna ägs i dessa filer - se kommentaren i Program.cs.
- **api:ns Spotify-credentials vid `aspire run`:** user secrets läses bara i Development, men `aspire run` startar api-resursen med `--no-launch-profile` (Production). På en maskin med creds i user secrets kraschar api:n med OptionsValidationException om inte credsen injiceras som env (`Spotify__ClientId`/`ClientSecret`/`BaseUrl`/`AccountsBaseUrl`) eller api-resursen får explicit Development-env (`WithEnvironment("ASPNETCORE_ENVIRONMENT","Development")`). Detta beteende är befintligt och oberoende av portarbetet.
...
- **Spotify Web API docs:** https://developer.spotify.com/documentation/web-api (referens + koncept) och den auktoritativa OpenAPI-specen https://developer.spotify.com/reference/web-api/open-api-schema.yaml (primär källa för endpoint-fakta - verifiera alltid limit- och parametervärden där, inte i gamla guides). Aktuella tak (verifierat 2026-09-22): search och get-an-artists-albums har limit max 10 (default 5) - högre ger HTTP 400 "Invalid limit"; get-followed/recently-played ligger kvar på max 50. Appen skickar aldrig limit > 10 på search/albums. Sökfiltret `label:"<namn>"` (typ album) fungerar mot riktig Spotify - empiriskt verifierat 2026-09-22 (label:"Globuli", label:"Ninja Tune" ger album-träffar server-seitigt) men är INTE listat i specens filterlista; q byggs som `label:"<namn>"` och enkodas med EscapeDataString (q=label%3A%22...%22).
